    ...
    func getNoticeListApi() {
        ...
        isNetworkAvailable { (isSuccess) in
            if Util.isNetworkReachable() {
                self.showProgress()
                self.objNoticeListViewModel.getNoticeListAPI()
            }else {
...
//        cell.lblMsgTitle.setBoldEditProfileUIStylePink(value: "Title message \(indexPath.row)")
        ...
        cell.lblMsgTitle.setNoticeTitleStyle(value: objNoticeList.notice_title)
        
        let date = Util.convertDateFormat(date: objNoticeList.notice_date, dateFormat: Constant.DateFormat.DateFormatYYYY_MM_DD_HH_MM_SS, newDateFormat: Constant.DateFormat.Simple_Date_Format)
...
//        objWebViewChargingStationFreeVC.strWebURL = objNoticeList.web_view_url
//        objWebViewChargingStationFreeVC.isNoticeDetailWebview = true
